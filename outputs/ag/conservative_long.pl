%%conservative_long weight: 10 timeout: 8000 recall_threshold: 0.8
%%sneeze
...
%%throw
%%Solution recall (0.47) below threshold 0.8
throw_target(_).
...
%%work
work_target(A):- touching(A,B),table(B).
...
%%take
%%Solution recall (0.74) below threshold 0.8
take_target(_).
...
%%hold
%%Solution recall (0.55) below threshold 0.8
hold_target(_).

%%awaken
awaken_target(A):- lying_on(A,C),person(B),lying_on(B,C).
...
%%put
%%Solution recall (0.20) below threshold 0.8
put_target(_).
...
%%tidy
%%Solution recall (0.54) below threshold 0.8
tidy_target(_).
