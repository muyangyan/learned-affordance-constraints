...
from util.box_ops import box_xywh_to_xyxy
...
class AG(Dataset):
    ...
    def __init__(self, config, transforms, mode, datasize, data_path=None, single_frame=False, filter_nonperson_box_frame=True, filter_small_box=False):
        ...
        if datasize != 'full':
            ...
            small_person = {}
            small_object = {}
            for i in list(person_bbox.keys())[:subsize]:
        ...
        self.frame_list = []
        self.frame_video_map = []
        self.gt_annotations_frames = []
            ...
            for j in video_dict[i]:
                if filter_nonperson_box_frame:
                    if person_bbox[j]['bbox'].shape[0] == 0:
                        ...
                        self.frame_list.append(j)
                        self.frame_video_map.append(i)
                ...
                self.gt_annotations_frames.append(gt_annotation_frame)
    ...
    def __getitem__(self, index):
        ...
        if self.single_frame:
            ...
            im = imread(os.path.join(self.frames_path, frame)) # channel h,w,3
            ...
            ##################### Load the image data
            frame = self.frame_list[index]
            image_unpadded = Image.open(os.path.join(self.frames_path, frame)).convert('RGB')
            w, h = image_unpadded.size

            annotations = self.gt_annotations_frames[index]

            ##################### Load the box location data
            gt_boxes = [annotations[0]['person_bbox'][0]] + [annot['bbox'] for annot in annotations[1:]]
            gt_boxes = np.stack(gt_boxes)
            gt_classes = np.stack([0] + [annot['class'] for annot in annotations[1:]])

            ##################### Load the relationship data
            gt_rels = []
            for i,annot in enumerate(annotations[1:]):
                for rel in annot['attention_relationship']:
                    gt_rels.append([0, i+1, rel])
                for rel in annot['spatial_relationship']:
                    gt_rels.append([i+1, 0, rel])
            ...
            gt_boxes = torch.as_tensor(gt_boxes, dtype=torch.float)
            target["boxes"] = box_xywh_to_xyxy(gt_boxes)
